...
public sealed class TreasuryDiscriminatorExporter
{
    ...
    public TreasuryDiscriminatorExporter()
        : this(() => DateTimeOffset.Now)
    {
    }

    public TreasuryDiscriminatorExporter(Func<DateTimeOffset> clock)
    {
        _clock = clock;
    }
        ...
        string? donjHackRoot = null)
    {
        ...
        var exportDirectory = DonjHackPathResolver.ResolveDiscriminatorDirectory(donjHackRoot);
        Directory.CreateDirectory(exportDirectory);

        var payload = new TreasuryDiscriminatorExportEnvelope(
            CreatedAt: _clock(),
            Process: process,
    ...
    private string WriteWithoutOverwrite(string exportDirectory, TreasuryDiscriminatorExportEnvelope payload)
    {
        ...
        for (var suffix = 0; suffix < 1000; suffix++)
        {
            var fileName = suffix == 0
                ? $"treasury-discriminator-{timestamp}.json"
                : $"treasury-discriminator-{timestamp}-{suffix:000}.json";
        ...
        throw new IOException($"Impossible de creer un export discriminator unique dans {exportDirectory}.");
    }
}
